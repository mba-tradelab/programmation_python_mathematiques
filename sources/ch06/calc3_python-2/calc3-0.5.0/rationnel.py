...
class rationnel(object):
	...
	def __init__(self, num =int(0), denom =int(1), valide =True):
		""" constructeur """
		...
		if valide:
			if type(num) == int or type(num) == int:
				num = entier(num)
			if type(denom) == int or type(denom) == int:
				denom = entier(denom)
		...
		if self.__valide:
			...
			self.__num, self.__denom = entier(num.valeur() // d.valeur()), entier(denom.valeur() // d.valeur())
		else:
